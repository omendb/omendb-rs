...
impl NodeStorage {
    ...
    /// Serialize the auxiliary body: SQ8 params, norms, sums, legacy PQ,
    /// RaBitQ state, and upper neighbors. Shared by both `serialize_auxiliary`
    /// and `serialize_full`.
    ...
    /// RaBitQ state, and upper neighbors. Shared by both `deserialize_auxiliary`
    ...
    fn deserialize_aux_body(&mut self, data: &[u8], pos: &mut usize) -> Result<(), String> {
        ...
        let (rabitq_trained, rabitq_params, rabitq_codes, rabitq_metadata, rabitq_originals) =
            if *pos + 2 <= data.len() && self.mode == StorageMode::RaBitQ {
                let trained = read_u8(data, pos)? != 0;
                let has_params = read_u8(data, pos)? != 0;
                let params = if has_params {
                    let param_len = read_u64(data, pos)? as usize;
                    let param_bytes = read_bytes(data, pos, param_len)?;
                    Some(
                ...
                for _ in 0..meta_len {
                    metadata.push(read_f32(data, pos)?);
                }
            ...
            } else if *pos + 2 <= data.len() && self.mode != StorageMode::RaBitQ {
        ...
        if self.mode == StorageMode::RaBitQ && !rabitq_trained && !rabitq_originals.is_empty() {
    ...
    pub fn serialize_auxiliary(&self) -> Vec<u8> {
        let mut out = Vec::with_capacity(256 + self.norms.len() * 4 + self.rabitq_codes.len() * 8);
    ...
    pub fn serialize_full(&self) -> Vec<u8> {
        ...
        // Auxiliary body (SQ8 params, norms, RaBitQ state, upper neighbors)
        self.serialize_aux_body(&mut out);
    ...
    pub fn deserialize_full(data: &[u8]) -> Result<Self, String> {
        ...
        // Set mode and trained before deserializing aux body (mode is checked inside)
        storage.mode = mode;
        storage.sq8_trained = sq8_trained;

        // Deserialize auxiliary body (SQ8 params, norms, RaBitQ state, upper neighbors)
