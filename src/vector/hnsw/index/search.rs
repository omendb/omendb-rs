...
impl HNSWIndex {
    ...
    ///
    /// # Reference
    /// Ada-ef: Distribution-Aware Adaptive ef, arXiv:2512.06636 (SIGMOD 2026)
    #[instrument(skip(self, query), fields(k, min_ef, max_ef, dimensions = query.len(), index_size = self.len()))]
    pub fn search_adaptive(
        ...
        // Validate inputs
        if k == 0 {
            return Err(HNSWError::InvalidSearchParams { k, ef: min_ef });
        }
        if min_ef < k {
            return Err(HNSWError::InvalidSearchParams { k, ef: min_ef });
        }
        ...
        // Greedy search at upper layers (same as standard search)
        ...
        // Collect initial distances from layer 0 neighbors for ef estimation
        let initial_distances = self.collect_initial_distances(query, &nearest)?;
        let adaptive_ef = Self::compute_adaptive_ef(&initial_distances, k, min_ef, max_ef);

        debug!(
            adaptive_ef,
            min_ef,
            max_ef,
            cv = Self::compute_cv(&initial_distances),
            "Ada-ef selected"
        );

        // Beam search at layer 0 with adaptive ef
        let candidates = if use_asymmetric {
            self.search_layer_asymmetric(query, &nearest, adaptive_ef.max(k), 0)?
        } else {
            self.search_layer(query, &nearest, adaptive_ef.max(k), 0)?
        };
    ...
    /// Collect initial distances from neighbors of entry points at layer 0
    ///
    /// Used by Ada-ef to estimate query "hardness" from distance variance.
    fn collect_initial_distances(&self, query: &[f32], entry_points: &[u32]) -> Result<Vec<f32>> {
        let mut distances = Vec::with_capacity(64);

        for &ep in entry_points {
            // Distance to entry point itself
            let ep_dist = self.distance_cmp(query, ep)?;
            distances.push(ep_dist);

            // Distances to neighbors of entry point (limit to 8 for minimal overhead)
            let neighbors = self.neighbors.get_neighbors(ep, 0);
            for &neighbor_id in neighbors.iter().take(8) {
                if let Ok(dist) = self.distance_cmp(query, neighbor_id) {
                    distances.push(dist);
                }
            }
        }
    ...
    /// Compute coefficient of variation (CV) from distances
    ///
    /// CV = std_dev / mean. High CV indicates spread-out distances (hard query).
    #[cfg_attr(test, allow(dead_code))]
    pub(crate) fn compute_cv(distances: &[f32]) -> f32 {
    ...
    /// Compute adaptive ef based on distance variance (Ada-ef simplified heuristic)
    ///
    /// Maps coefficient of variation to ef:
    /// - Low CV (clustered results) → low ef (easy query)
    /// - High CV (spread results) → high ef (hard query)
    pub(crate) fn compute_adaptive_ef(
        distances: &[f32],
        k: usize,
        min_ef: usize,
        max_ef: usize,
    ) -> usize {
        let cv = Self::compute_cv(distances);

        // Base ef is k * 10 (standard HNSW heuristic)
        // Scale by (1 + cv * 2): cv=0 → 1x, cv=0.5 → 2x, cv=1.0 → 3x
        let base_ef = k * 10;
        let scaled_ef = (base_ef as f32 * (1.0 + cv * 2.0)) as usize;

        scaled_ef.clamp(min_ef, max_ef)
    }
