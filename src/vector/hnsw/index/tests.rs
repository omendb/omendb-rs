...
// ============================================================================
// RaBitQ Integration Tests
// ============================================================================

#[test]
fn test_rabitq_insert_beyond_training_threshold() {
    // Insert 500 vectors — triggers training at 256
    let params = HNSWParams {
        m: 16,
        ef_construction: 100,
        ..Default::default()
    };
    let mut index = HNSWIndex::new_with_rabitq(128, params, DistanceFunction::L2).unwrap();
    ...
    for (i, v) in vectors.iter().enumerate() {
        ...
        assert_eq!(id, i as u32, "Insert #{i} should succeed");
    }

    assert_eq!(index.len(), 500);
    assert!(index.is_rabitq());
}

#[test]
fn test_rabitq_search_recall() {
    // Insert 1000 vectors, search, verify recall
    let params = HNSWParams {
        m: 16,
        ef_construction: 100,
        ..Default::default()
    ...
    for q_idx in 0..num_queries {
        ...
        assert!(
            !results.is_empty(),
            "Search should return results for query {q_idx}"
        );

        // Brute-force ground truth
        let mut exact: Vec<(u32, f32)> = (0..vectors.len() as u32)
            .map(|i| {
                let dist: f32 = query
                    .iter()
                    .zip(vectors[i as usize].iter())
                    .map(|(a, b)| (a - b) * (a - b))
                    .sum::<f32>()
                    .sqrt();
                (i, dist)
    ...
    // Raw 1-bit quantization on uniform random 128D data achieves ~40% recall@10.
    // Production recall (>95%) comes from oversampling + rescore at the VectorStore layer.
    assert!(
        avg_recall >= 0.3,
        "RaBitQ recall@{k} too low: {:.1}%",
        avg_recall * 100.0
    );
}

#[test]
fn test_rabitq_persistence_roundtrip() {
    let params = HNSWParams {
        m: 16,
        ef_construction: 100,
        ..Default::default()
    };
    let mut index = HNSWIndex::new_with_rabitq(64, params, DistanceFunction::L2).unwrap();
    ...
    assert!(loaded.is_rabitq());

    // Search on loaded index should work
    let results = loaded.search(&vectors[0], 5, 100).unwrap();
    assert!(!results.is_empty());
    // Self-search should find the original vector
    assert!(
        results[0].distance < 1.0,
        "Self-search distance too high: {}",
        results[0].distance
    );
}
