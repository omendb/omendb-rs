...
fn test_golden_hybrid_ranking_stability() {
    ...
    let docs = [
        (0, "the quick brown fox jumps over the lazy dog [animal]", json!({"tag": "animal"})),
        (1, "machine learning algorithms for vector databases [tech]", json!({"tag": "tech"})),
        (2, "rust programming language is fast and safe [tech]", json!({"tag": "tech"})),
        (3, "cooking recipes for delicious apple pie [food]", json!({"tag": "food"})),
        (4, "deep learning models and neural networks [tech]", json!({"tag": "tech"})),
    ];

    for (id_num, text, metadata) in &docs {
        let id = format!("doc_{id_num}");
        let vec = golden_vector(*id_num, 128);
        store.set_with_text(&id, vec, text, metadata.clone()).unwrap();
    }
    ...
    assert_eq!(results[0].0, "doc_1", "Doc 1 should rank first (exact vector match + text match)");
    assert_eq!(results[1].0, "doc_4", "Doc 4 should rank second (text match + some vector similarity)");
...
fn test_golden_filtered_search_stability() {
    ...
    for i in 0..100 {
        ...
        store.set(&id, vec, json!({"cat": category, "val": i})).unwrap();
    }
