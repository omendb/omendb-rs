...
fn main() -> anyhow::Result<()> {
    ...
    // Create and populate store
    {
        ...
        store.set("a".into(), Vector::new(vec![1.0; 64]), json!({"name": "first"}))?;
        store.set("b".into(), Vector::new(vec![2.0; 64]), json!({"name": "second"}))?;
        store.set("c".into(), Vector::new(vec![3.0; 64]), json!({"name": "third"}))?;
