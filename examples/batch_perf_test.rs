...
fn main() {
    ...
    println!("Single-query: {:.0} QPS ({:.2}ms per 100q)", single_qps, single_elapsed.as_secs_f64() * 1000.0 / iters as f64);
    println!("Batch (parallel): {:.0} QPS ({:.2}ms per 100q)", batch_qps, batch_elapsed.as_secs_f64() * 1000.0 / iters as f64);
